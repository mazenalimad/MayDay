...
export default ({
  ...
  const [tabMeasurements, setTabMeasurements] = useState(
    state.routes.map(() => ({ x: 0, y: 0, width: 0, height: 0 }))
  );

  useEffect(() => {
    const tms = [];
    tab_refs.forEach((tr, i, arr) => {
      tr.current.measureLayout(
        tab_container_ref.current,
        (x, y, width, height) => {
  ...
  flatlist_ref.current?.scrollToOffset({
    offset: state.index ? tabMeasurements[state.index].x - 50 : 0,
    animated: true,
  });

  return (
    <Animated.View
      style={[song_details_tab_bar_container, { opacity: global_opacity }]}
    >
      <View
        ref={tab_container_ref}
        style={song_details_tab_indicators_container}
      >
        <Animated.View
          style={[
            song_details_tab_indicator_background,
            {
              backgroundColor:
                accent_color ||
                song_details_tab_indicator_background.backgroundColor,
              width: Animated.add(
                width,
                new Animated.Value(TAB_INDICATOR_BACKGROUND_HORIZONTAL_PADDING)
              ),
              transform: [
                {
                  translateX: Animated.sub(
                    translateX,
                    new Animated.Value(
                      TAB_INDICATOR_BACKGROUND_HORIZONTAL_PADDING / 2
                    )
                  ),
                },
                {
                  translateY: -(TAB_INDICATOR_BACKGROUND_VERTICAL_PADDING / 2),
                },
              ],
              height:
                tabMeasurements[0].height +
                TAB_INDICATOR_BACKGROUND_VERTICAL_PADDING,
              left,
            },
          ]}
        />
          ...
          renderItem={({ item: route, index }) => {
            ...
            return (
              <Animated.View
                ref={tab_ref}
                key={route.key}
                style={song_details_tab}
              >
                <TouchableOpacity onPress={onPress}>
                  <Text
                    style={{
                      color: song_details_tab.color,
                      fontSize: song_details_tab.fontSize,
                      alignSelf: "center",
                    }}
                  >
                    {label.toUpperCase()}
