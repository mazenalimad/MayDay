...
const equalizer_heights = equalizer_lines.map(
  () => new Animated.Value(equalizer_line.height * 0.3)
);

const equalizer_scale = new Animated.Value(0);

const equalizer_animations = Animated.loop(
  Animated.parallel(
    equalizer_heights.map((height, i) => {
      ...
      return Animated.sequence([
        Animated.timing(height, {
          toValue: equalizer_line.height,
          duration,
          delay,
          useNativeDriver: false,
        }),
        Animated.timing(height, {
          toValue: equalizer_line.height * 0.3,
          duration,
          delay: 0,
          useNativeDriver: false,
        }),
...
const equalizer_show_animation = Animated.timing(equalizer_scale, {
  toValue: 1,
  duration: 200,
  useNativeDriver: true,
});

const equalizer_hide_animation = Animated.timing(equalizer_scale, {
  toValue: 0,
  duration: 200,
  useNativeDriver: true,
});
...
const EqualizerButton = ({ playing = false, loading_song = null, onPress }) => {
  ...
  return (
    <Animated.View
      pointerEvents={playing || loading_song ? "auto" : "none"}
      style={[equalizer_overlay, { transform: [{ scale: equalizer_scale }] }]}
    >
      <TouchableHighlight onPress={onPressEqualizer}>
        <View
          style={{
            width: equalizer_overlay.width,
            height: equalizer_overlay.height,
            ...
            return (
              <Animated.View
                ...
                style={[
                  ...
                  {
                    height: height,
                    backgroundColor: loading_song
                      ? "rgba(200,200,200,1)"
                      : equalizer_line.backgroundColor,
                  },
                ]}
              />
            );
